...
!   Emissions Inventory files
    data efile /'INH3_2018.csv','INOx_2018.csv','ISO2_2018.csv',&
&           'IVOC_2018.csv','ICO__2018.csv','IPM10_2018.csv',&
&           'IPM25_2018.csv','ICO2_2018.csv','IBC__2018.csv',&
&           'imet__2018.csv'/
!            NH3          NO2         SO2    VOC    CO PM10 PM25
    data ofile /'ANH3_2018.csv','ANOx_2018.csv','ASO2_2018.csv',&
&           'AVOC_2018.csv','ACO__2018.csv','APM10_2018.csv',&
&          'APM25_2018.csv','ACO2_2018.csv','ACN__2018.csv', &
&          'ACH4_2018.csv'/
...
!    if(scc(k,l).eq.'2270005000') eagr(j,k,l)=emiss(i,l,k)*fa(j)*1e6  ! Comb_agricola_Diesel
    if(scc(k,l).eq.'2805020000') eagr(j,k,l)=emiss(i,l,k)*fa(j)*1e6  ! Ganaderas NH3
    if(scc(k,l).eq.'2805000000') eagr(j,k,l)=emiss(i,l,k)*fa(j)*1e6  ! Ganaderas PM
...
!$omp section
    ...
    if(scc(k,l).eq.'2701200000') ebos(j,k,l)=emiss(i,l,k)*fb(j)*1e6 ! Usar las del modelo.
            end do
            exit invenb
         end if
        end do invenb
    ...
    pmar: do j=1,size(fm) !grid
        ...
        if(idm(j).eq.iem(k,i)) then
            do l=1,nscc(k)
                ...
                if(scc(k,l).eq.'2280002000') epue(j,k,l)=emiss(i,l,k)*fm(j)*1e6 !Embarcaciones marinas
            end do
            exit invpm
        end if
        end do invpm
...
!dir$ loop count min(16)
    ...
    if(scc(k,l).eq.  '30500399') epob(j,k,l)=emiss(i,l,k)*fp2(j)*1e6  ! Ladrillera
    ...
    if(scc(k,l).eq.'2401002000') epob(j,k,l)=emiss(i,l,k)*fp3(j)*1e6  ! Recub Industriales
    ...
    if(scc(k,l).eq.'2465200000') epob(j,k,l)=emiss(i,l,k)*fp3(j)*1e6  ! Productos uso solvente domestico
    ...
    if(scc(k,l).eq.'2325000000') epob(j,k,l)=emiss(i,l,k)*fp3(j)*1e6  ! Bancos de Materiales
     end do
            exit invenp
        end if
    end do invenp
