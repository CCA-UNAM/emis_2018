...
character (len=19) :: current_date='2018-01-01_00:00:00'
 !> Input file name
...
character (len=40) :: titulo
 ...
 data efile/'ASO2_2018.csv','ANOx_2018.csv','ANH3_2018.csv',&
&           'ACO__2018.csv','APM10_2018.csv','ACO2_2018.csv',&
&           'ACN__2018.csv','ACH4_2018.csv','APM25_2018.csv',&
&           'AVOC_2018.csv'/
 data casn /'MEX_AREA_9km_2018_SO2.nc ','MEX_AREA_9km_2018_NOx.nc ',&
&           'MEX_AREA_9km_2018_NH3.nc ','MEX_AREA_9km_2018_CO.nc  ',&
&           'MEX_AREA_9km_2018_PM10.nc','MEX_AREA_9km_2018_CO2.nc ',&
&           'MEX_AREA_9km_2018_BC.nc  ','MEX_AREA_9km_2018_CH4.nc ',&
&           'MEX_AREA_9km_2018_PM25.nc','MEX_AREA_9km_2018_VOC.nc '/
  ...
  do k=1,nf
    do ii=1,size(idcel2)
        ...
        if(idcel2(ii).eq.id5(k,i))then
            ...
            if(iscc(j,k).eq.'2701200000') emis(ii,k,16)=emis(ii,k,16)+emiA(k,i,j)
            if(trim(iscc(j,k)).eq.'30500304') emis(ii,k,10)=emis(ii,k,10)+emiA(k,i,j)
            if(trim(iscc(j,k)).eq.'30500399') emis(ii,k,10)=emis(ii,k,10)+emiA(k,i,j)
            if(iscc(j,k).eq.'2280000000') emis(ii,k,11)=emis(ii,k,11)+emiA(k,i,j)
            if(iscc(j,k).eq.'2280002000') emis(ii,k,11)=emis(ii,k,11)+emiA(k,i,j)
            ...
            if(iscc(j,k).eq.'2270008010') emis(ii,k,15)=emis(ii,k,15)+emiA(k,i,j)
            ...
            if(iscc(j,k).eq.'2805000000') emis(ii,k,1)=emis(ii,k,1)+emiA(k,i,j)
            ...
            if(iscc(j,k).eq.'2401002000') emis(ii,k,10)=emis(ii,k,10)+emiA(k,i,j)
...
call check( nf90_put_att(ncid, NF90_GLOBAL, "id","AE_2018_"//trim(ename(k))))
call check( nf90_put_att(ncid, NF90_GLOBAL, "title","Emissions from criteria pollutants and GHG for 2018"))
...
call check( nf90_put_att(ncid, NF90_GLOBAL, "time_coverage_start","2018-01-01T06:00:00Z"))
...
call check( nf90_put_att(ncid, NF90_GLOBAL, "summary","National emissions inventory for Mexico 2018&
...
call check( nf90_put_att(ncid, NF90_GLOBAL, "date_modified",hoy))
call check( nf90_put_att(ncid, NF90_GLOBAL, "comment","Created with areaanual.F90 v1.1"))
...
call check( nf90_put_att(ncid, id_unlimit, "units", "years since 2018-01-01T06:00:00Z"))
...
!Tiempo
    call check( nf90_put_var(ncid, id_dim(1),"2018-01-01_06:00:00"))! one year output
